...
- `astro.config.mjs` の `site` は `https://sol-momma-world.sol0608sol.workers.dev`
...
- **ホスティング**: Cloudflare Workers（静的サイト配信）
- **URL**: `https://sol-momma-world.sol0608sol.workers.dev`
- **CI/CD**: GitHub Actions（`.github/workflows/deploy.yml`）— `main` push 時に `pnpm build` → `wrangler deploy`
- **インフラ管理**: Terraform（`infra/`）— Cloudflare プロバイダー。カスタムドメイン追加時に拡張
- **Workers 設定**: `wrangler.jsonc` で静的アセット配信を定義
